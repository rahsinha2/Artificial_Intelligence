# Submitted by : Rahul Sinha      rahsinha@indiana.edu
...
def breadthFirst(queue,visited):
    ...
    toExpand = heappop(queue)
    #print"poped-->",toExpand[1].hand['location']
    if toExpand[1].isGoal():
	return queue, toExpand[1]
    priority = toExpand[0]
    priority = priority + 1
#    print toExpand[1].hand['location'] 
#    visited.append(toExpand[1])
    stack = []
    for i in range(0, len(queue)):
	temp = queue[i]
	stack.append(temp[1])	
  #  for element in stack:
#	print element.hand['location']
   # print "--------------------------------------------------------------------------------------"
	
    for action in toExpand[1].allowedActions():
	neighbor = copy.deepcopy(toExpand[1])
        deltaCost = neighbor.do(action)
        if not duplicateWorld(neighbor,visited):
            if not duplicateWorld(neighbor,stack):
		#if not (elements[1].hand == neighbor.hand and elements[1].blocks == neighbor.blocks) :
		#visited.append(neighbor)
                heappush(queue,(priority, neighbor))
 #  	        print "pushed ---->", neighbor.hand['location']
    #print "========================================================================================="
...
def aStar(queue,visited,heuristic):
	...
	neighbor = copy.deepcopy(toExpand[1])
        deltaCost = neighbor.do(action)
 	neighbor_priority = neighbor.cost + heuristic(neighbor)
        if not duplicateWorld(neighbor,visited):
            if not duplicateWorld(neighbor,stack):
                heappush(queue,(neighbor_priority, neighbor))
...
def bestFirst(queue,visited,heuristic):
	...
	neighbor = copy.deepcopy(toExpand[1])
        deltaCost = neighbor.do(action)
 	neighbor_priority = heuristic(neighbor)
        if not duplicateWorld(neighbor,visited):
            if not duplicateWorld(neighbor,stack):
                heappush(queue,(neighbor_priority, neighbor))
...
def h_manhattan(world):
    sum = 0
    for block in world.blocks.iteritems():
	name1, (a,b,c) = block
        (x,y,z) = world.goal[name1]
        sum = sum + (abs(x-a) + abs(y-b))			
    return sum

def h_euclidean(world):
    sum = 0
    for block in world.blocks.iteritems():
	name1, (a,b,c) = block
        (x,y,z) = world.goal[name1]
        sum = sum + math.sqrt(((x-a)**2 )+ ((y-b)**2))			
    return sum


hs = {h_manhattan, h_euclidean} 	#this is a set, put your heuristic functions in here.

presolved = RobotWorld(4,4,1,{'A' : (3,3,0), 'B' : (3,1,0)}, {'A' : (3,3,0), 'B' : (3,1,0)})
# run(presolved,'presolved',hs)
easy = RobotWorld(5,5,1,{'A' : (0,0,0), 'B' : (3,1,0)}, {'A' : (1,1,0), 'B' : (3,1,0)})
#run(easy,'easy',hs)
